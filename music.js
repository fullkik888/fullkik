...
app.post('/api/users/:username/purchase', async (req, res) => {
    try {
        ...
        if (user.tokens >= price) {
            ...
            // Ensures the correct uploader name is logged for the Purchase History Tab
            user.purchases.push({ 
                songId: req.body.songId, songName: song.filename, filepath: song.filepath, coverUrl: song.coverUrl, 
                uploader: song.uploader || 'FULL KIK', 
...
async function saveSongData(fileBuffer, originalName, reqBody) {
    ...
    const newSong = {
        ...
        downloads: 0, plays: 0, status: reqBody.status || 'APPROVED', uploader: reqBody.uploader || 'FULL KIK'
    };
...
app.get('/api/settings', async (req, res) => {
    if(!db) return res.json({ headerTitle: 'FULL KIK', heroTitle: '专属DJ节奏空间', bannerUrl: '' });
    const doc = await db.collection('settings').doc('global').get(); res.json(doc.exists ? doc.data() : { headerTitle: 'FULL KIK', heroTitle: '专属DJ节奏空间', bannerUrl: '' });
});
